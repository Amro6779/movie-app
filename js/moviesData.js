...
export async function getMovie(term) {
    let myHttp = await fetch(`https://api.themoviedb.org/3/${term}?api_key=eba8b9a7199efdcb0ca1f96879b83c44&language=en-US&include_adult=false`);
...
export async function searchMovie(term) {
    let myHttp = await fetch(`https://api.themoviedb.org/3/search/movie?query=${term}&api_key=eba8b9a7199efdcb0ca1f96879b83c44&language=en-US&include_adult=false`);
    if (myHttp.ok && 400 != myHttp.status) {
        let Data = await myHttp.json();
        results = Data.results;
        movies = new Map(Object.entries(results));
